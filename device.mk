...
PRODUCT_COPY_FILES += $(foreach device,camellian camellianp, \
    ...
    frameworks/native/data/etc/android.hardware.nfc.hcef.xml:$(TARGET_COPY_OUT_ODM)/etc/permissions/sku_$(device)/android.hardware.nfc.hcef.xml \
    frameworks/native/data/etc/android.hardware.nfc.uicc.xml:$(TARGET_COPY_OUT_ODM)/etc/permissions/sku_$(device)/android.hardware.nfc.uicc.xml)
...
# Shipping API level
# camellia launched on Android 11, so the shipping API level is 30.
PRODUCT_SHIPPING_API_LEVEL := 30

# The stock vendor image is Android 12 and declares ro.vndk.version=31
# (confirmed in vendor build.prop; its AVB fingerprint is
# Redmi/camellia/camellia:12/SP1A.210812.016). 582 of the shipped vendor blobs
# link against VNDK libraries, so the system image must carry the v31 snapshot
# or every one of those HALs fails to load at runtime.
PRODUCT_EXTRA_VNDK_VERSIONS := 31

# SKU
PRODUCT_COPY_FILES += \
    $(call find-copy-subdir-files,*,$(LOCAL_PATH)/configs/sku/,$(TARGET_COPY_OUT_VENDOR)/etc)
...
# installed as init.mt6833.usb.rc) READS vendor.usb.controller but nothing sets
# it, so init substitutes the default "16751000.usb0" - a UDC that does not
# exist on camellia. The real one is musb-hdrc, from
# platform_device_alloc("musb-hdrc", ...) in
# drivers/misc/mediatek/usb20/mt6833/usb20.c. Without this the gadget never
# binds: no ADB, no MTP, no tethering, no OTG host.
