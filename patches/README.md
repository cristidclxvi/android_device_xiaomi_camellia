...
| `sensors_multihal_vendor_type.patch` | `hardware/interfaces` | The sensors HAL crash-loops at boot. `convertToAidlEvent()` switches on the sensor type and its `default:` arm asserts `CHECK_GE(sensorType, DEVICE_PRIVATE_BASE)` (`sensors/aidl/default/multihal/ConvertUtils.cpp:411`), i.e. anything it does not recognise must be a device-private type at or above 65536. Xiaomi's sensor stack emits vendor type **75**, far below that, so the first such event turns `CHECK_GE` into `LOG(FATAL)` and aborts `android.hardware.sensors-service.xiaomi-multihal`. init restarts it, the queued event is redelivered, and it dies again - every tombstone reads `Process uptime: 0s`. The assert guards nothing: the payload copy immediately below already handles an arbitrary type by memcpy-ing the raw values, which is exactly what a device-private event gets. Dropping the assert lets the event through unchanged. Reported downstream as issue #1. |
